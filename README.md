...
Sound lists every speaker with one tick and one balance slider. Unticked speakers stay silent and leave the menu panel. The balance slider keeps a speaker permanently below the rest: at 80 percent it always sits 20 percent under the others while everything rises and falls together, useful when one speaker is naturally louder. A monitor appears as a single speaker even though macOS sees its audio and its volume control as separate things.

Sound mode chooses how the mix is built. Stereo plays every ticked device with its natural left and right. Mono plays the complete mix on every speaker, the right choice when speakers sit in different places or rooms and each should carry the whole sound rather than one side of it. Spatial opens an arrangement view instead: drag every speaker to where it stands in your room, in a full circle around you and at any distance, and the macOS spatial mixer renders the matching part of the stereo field to each one. Nearer speakers are delayed a fraction so all sound arrives at your seat together; speakers behind you work, height does not, because stereo carries no height information. The Reset button returns every speaker to its natural stereo side. The speaker ticks choose which devices take part in playback at all, so Audio MIDI Setup is never needed. While a single device is selected in the sound output list, the sound mode options are disabled and the other speakers dim; everything comes back when Unison is selected again.

Displays works the same way for brightness: a tick per display and a balance slider that keeps one display dimmer than the others at every level.
