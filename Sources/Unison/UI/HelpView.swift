        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                section("Why it exists",
                    "macOS plays sound to one output device at a time, and combining devices by hand in Audio MIDI Setup disables the volume keys. Monitors connected over HDMI or DisplayPort expose no volume or brightness control to macOS at all. Unison fills these gaps: it plays sound through every ticked device at once, drives each speaker directly, and talks to monitors over DDC, the protocol monitors use for their own menu settings.")

                section("Playing through several devices",
                    "Unison appears in the sound output list as a device of its own. While Unison is selected, the sound your Mac plays is captured, using the System Audio Recording permission, and redistributed to every speaker ticked in Settings. In Spatial mode, open Arrange Speakers and drag each speaker to where it physically stands, anywhere around you including behind; the macOS spatial mixer renders the matching part of the stereo field to it, and nearer speakers are delayed slightly so everything arrives at your seat together. In Stereo mode, every device keeps its natural stereo. In Mono mode, every speaker plays the complete mix, which suits speakers spread across different rooms.")
                ...
                section("Using a second display",
                    "External monitors appear automatically in both columns of the menu panel if they support DDC, and most do. Brightness works on nearly every monitor; volume only on monitors with speakers. If a monitor is missing, click Refresh Devices in the menu panel, and check it is connected directly rather than through a hub or KVM.")

                section("Keeping devices in balance",
                    "In Settings, each device has a balance slider. A speaker set to always 20% quieter follows every volume change but stays proportionally below the rest: at zero everything is silent, at full level it sits at 80%. The same works for displays with always dimmer. Use it when one speaker is louder than the other, or when one display should stay dimmer, for example next to a dark code editor.")
